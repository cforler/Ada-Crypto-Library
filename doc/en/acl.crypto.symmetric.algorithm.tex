...
An iterative hash function $H$ decomposes a message $M$ into $r$ message blocks $m_i$ ($1\leq i \leq r$) of length $n$. All message blocks will be hashed sequentially. To generate a hash value $h$ for the complete message $M$, the intermediate outputs are also inputs for the next round. %That means, hash value of the $i$-th message block is dependent on $h_1,\cdots,h_{i-1}$. 
In general there exists: $h_i:= H(m_i,h_{i-1})$. For $i=1$, $h_1:=H(M_1,h_0)$ will be applied, hereby $h_0$ is an initial value, and will be initialized at the beginning. Messages whose length is not multiple of $n$ will be inflated by a function \texttt{Padding()} before encryption until the length is multiple of $n$.%, appended by a block, which indicate the message length.
%That is, the message which has a length of $j\cdot n$ will be inflated to a length of $(j+1)\cdot n$.
...
\end{itemize}
%\textbf{Exceptions:}\\ 
%If there is an error in opening a to be hashed data: \texttt{File\_Open\_Error};\\
%If there is an error in reading a to be hashed data: \texttt{File\_Read\_Error}.
...
\end{lstlisting}\\ 
%If a message, which is longer than $2^{64}-1024$ bits (about 2 million TB), is hashed using SHA-256: \texttt{SHA256\_Constraint\_Error}.\\
\textbf{Exceptions:}\\
...
\begin{lstlisting}{}
  ...
  function Final_Round
           (Last_Message_Block : DW_Block512;
            ...
            Hash_Value         : DW_Block512) return DW_Block512;
\end{lstlisting}\\
%If a message, which is longer than $2^{256}-1024$ bits (about $2^{80}$ TB), is hashed using \texttt{Whirlpool}: \texttt{Whirlpool\_Constraint\_Error}.\\
\textbf{Exceptions:}\\
