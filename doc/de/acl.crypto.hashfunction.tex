...
Mit Hilfe dieses generischen Paketes lässt sich aus dem Algorithmus einer
kryptographsichen Hashfunktion (siehe Kapitel \ref{algoh})
eine krypto. Hashfunktion erstellen die sich hervorragend für folgende Zwecke 
einsetzen:
\begin{itemize}
\item Integritätsüberprüfung von Nachrichten
\item Generierung und Verifizierung von digitalen Signaturen.
\item Generierung von Zufallszahlen bzw. Zufallsbits
\end{itemize}
Der Zweck dieses Paketes ist es die API für Hashfunktionen zu vereintheilichen
...
Hashwert der Datei \textit{Filename} zurück.  Beispielsweise liefert 
die Codezeile
\begin{lstlisting}{}
  H := F_Hash("/bin/ls")
\end{lstlisting}
...
\item Die Prozedur \textbf{Init} initialisiert bzw. reinitialisiert die 
  ...
  zunächst diese Prozedure aufgerufen werden.
  \begin{lstlisting}{}
 procedure Init;
  \end{lstlisting}

\item Mit der Prozedure \textbf{Round}, können iterativ Nachrichtenblöcke
  gehashed werden. 
  \begin{lstlisting}{}
procedure Round(Message_Block : in Message_Type);
 \end{lstlisting}
 
\item Die Funktion \textbf{Final\_Round} padded und hashed anschließend einen
  Nachrichtenblock \textit{Last\_Message\_Block}. Auf Grund des Paddings muss
  die Bytelänge des Nachrichtenmaterials \textit{Last\_Message\_Length} 
  angegeben werden.  Denn eine Nachricht ist i.d.R.  kürzer als eine 
  Nachrichtenblock vom Typ \textit{Message\_Type}. Der Rückgabewert dieser 
...
\section{Anmerkung}
Sie müssen nicht jedes mal von neuem aus einem dafür vorgesehenen symmetrischen
Algorithmus eine Hashfunktion generieren. Stattdessen können Sie auch eine der 
