...
package body Test.Big_Number_Pow_Mod_Utils is
...
------------------------------------------------------------------------------------
	...
	begin
		...
		Register_Routine(T, Big_Number_Mod_Utils_Test1'Access,"Powiplication with Modulo N = 2^4096-1.");
		Register_Routine(T, Big_Number_Mod_Utils_Test2'Access,"Powiplication with Modulo N = 2^3812.");
		Register_Routine(T, Big_Number_Mod_Utils_Test3'Access,"Powiplication with Modulo N = 2^2048.");
		Register_Routine(T, Big_Number_Mod_Utils_Test4'Access,"Powiplication with Modulo N = 2^1025.");
		Register_Routine(T, Big_Number_Mod_Utils_Test5'Access,"Powiplication with Modulo N = 2^1024.");
		Register_Routine(T, Big_Number_Mod_Utils_Test6'Access,"Powiplication with Modulo N = 2^768.");
		Register_Routine(T, Big_Number_Mod_Utils_Test7'Access,"Powiplication with Modulo N = 2^0.");
...
------------------------------------------------------------------------------------
   ...
   begin
   	   ...
   	   X := Pow(X_4096, X_4096, X_4096);
   	   Assert(X = X_0, "Failed with 4096 and 4096 Bit.");

--	   X := Pow(X_4096, X_4095, X_4096);
--   	   Assert(X = X_0, "Failed with 4096 and 4095 Bit.");
--	   
--	   X := Pow(X_4095, X_4096, X_4096);
--   	   Assert(X = 2, "Failed with 4095 and 4096 Bit.");
--	   
--   	   X := Pow(X_4096, X_3812, X_4096);
--   	   Assert(X = X_0, "Failed with 4096 and 3812 Bit.");

	   A := To_Big_Unsigned("2");
   	   X := Pow(X_3812, A, X_4096);
	   N := Mult(X_3812, X_3812, X_4096);
   	   Assert(X = N, "Failed with 3812 and 2 Bit.");
   	   
--	   A := To_Big_Unsigned("3");
--   	   X := Pow(X_3812, A, X_4096);
--	   N := To_Big_Unsigned(Number_41);
--   	   Assert(X = N, "Failed with 3812 and 3 Bit.");
--   	   
--   	   X := Pow(X_4095, X_3812, X_4096);
--   	   Assert(X = X_1, "Failed with 4095 and 3812 Bit.");
--   	   
--   	   X := Pow(X_3812, X_582, X_4096);
--	   N := To_Big_Unsigned(Number_42);
--   	   Assert(X = N, "Failed with 3812 and 582 Bit.");
--   	   
--   	   X := Pow(X_3812, X_2048, X_4096);
--	   N := To_Big_Unsigned(Number_40);
--   	   Assert(X = N, "Failed with 3812 and 2048 Bit.");
--   	   
--   	   X := Pow(X_3812, X_1025, X_4096);
--	   N := To_Big_Unsigned(Number_43);
--   	   Assert(X = N, "Failed with 3812 and 1025 Bit.");
   	   
--   	   X := Pow(X_2048, X_2048, X_4096);
--	   N := To_Big_Unsigned(Number_44);
--   	   Assert(X = N, "Failed with 2048 Bit.");
   	   
--   	   X := Pow(X_1025, X_2048, X_4096);
--	   N := To_Big_Unsigned(Number_45);
--   	   Assert(X = N, "Failed with 1025 and 2048 Bit.");
   	   
--   	   X := Pow(X_1024, X_2048, X_4096);
--	   N := To_Big_Unsigned(Number_46);
--   	   Assert(X = N, "Failed with 1024 and 2048 Bit.");
--   	   
--   	   X := Pow(X_768, X_3812, X_4096);
--	   N := To_Big_Unsigned(Number_47);
