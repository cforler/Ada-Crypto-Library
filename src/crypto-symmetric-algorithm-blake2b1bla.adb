...
package body Crypto.Symmetric.Algorithm.Blake2b1Bla is
...
begin
  ...
  a := fblamka(a, b) + (((m(blake2bSigma(r,2*i+0)))));
  d := Rotate_Right(d xor a, 32);
  c := fblamka(c, d);
  b := Rotate_Right(b xor c, 24);

  a := fblamka(a, b) + (((m(blake2bSigma(r,2*i+1)))));
...
end  Crypto.Symmetric.Algorithm.Blake2b1Bla;
